...
{
	$str=bin2hex($str);
	$str=preg_replace("/([\\x00-\\xFF]{2,2})/","\\x$1",$str);
	return $str;
}
...
class PhpUploader
{
	...
	function _GenerateAttribute($name,$type=null)
	{
		...
		return " $name='".htmlentities($value,ENT_QUOTES)."'";
	}
	...
	function _HandleAddonUpload()
	{
		...
		$this->SecurityCheckFileName($file["name"]);
	...
	function _HandleIFrameUpload()
	{
		...
		$this->_ValidateFile(PhpUploader_GetBaseName($file["name"]),$file["size"]);
	...
	{
		if(!@$file["tmp_name"])
			throw(new Exception("(1) tmp_name is not available for " . $file["name"]));
		...
		{
			PhpUploader_Log("try_upload_php_file");
			throw(new Exception("(1) fails to upload file : " . $file["name"]));
		}
		
		$dir = $this->InternalGetTempDirectory();
		$filepath="$dir/uploading." . $guid . "." . PhpUploader_GetBaseName($file["name"]) . ".resx";
		
		PhpUploader_Log(" to : $filepath");
		
		//throw(new Exception("Get file ".$file["tmp_name"]." - $filepath"));
	...
	{
		if(!$file["tmp_name"])
			throw(new Exception("(2) tmp_name is not available for " . $file["name"]));
		if(!PhpUploader_FileExists($file["tmp_name"]))
	...
	{
		$id = @$_GET['_UploadID'];
		if(stripos($id,"<")!=false)throw(new Exception("Invalid _UploadID"));
		if(stripos($id,">")!=false)throw(new Exception("Invalid _UploadID"));
		return $id;
	}  
